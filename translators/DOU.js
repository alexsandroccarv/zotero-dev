{
	...
	"label": "Diário Oficial da União, Brasil v1.03",
	...
	"lastUpdated": "2026-06-15 15:00:00"
}
...
function doWeb(doc, url) {
	...
	if (temCertificada) {
		// Diálogo: mostra a página inicial (vinda do metadado) e o usuário
		// escolhe a página final. As opções vão da inicial até inicial + 19.
		var opcoes = {};
		for (var fim = paginaInicial; fim < paginaInicial + 20; fim++) {
			opcoes[String(fim)] = (fim === paginaInicial)
				? (paginaInicial + " (somente a página inicial)")
				: (paginaInicial + " a " + fim + " (" + (fim - paginaInicial + 1) + " páginas)");
		}
		Zotero.selectItems(opcoes, function (selecao) {
			var paginaFinal = paginaInicial; // padrão: só a inicial
			if (selecao) {
				for (var chave in selecao) {
					var v = parseInt(chave, 10);
					if (v > paginaFinal) paginaFinal = v;
				}
			}
			finalizar(paginaFinal - paginaInicial + 1);
		});
	} else {
		finalizar(0);
	}
